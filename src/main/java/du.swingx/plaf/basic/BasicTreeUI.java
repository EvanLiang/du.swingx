...
public class BasicTreeUI extends TreeUI {
    ...
    public void paint(Graphics g, JComponent c) {
        ...
        if (initialPath != null && paintingEnumerator != null) {
            ...
            //Draw background for the row selected
            int r = row;
            Color org = g.getColor();
            for (TreePath path : list) {
                bounds = getPathBounds(path, insets, boundsBuffer);
                if (bounds == null) continue;
                if (tree.isRowSelected(r)) {
                    Color bg;
                    DefaultTreeCellRenderer dtcr = new DefaultTreeCellRenderer();
                    TreeCellRenderer tcr = tree.getCellRenderer();
                    if (tcr instanceof JXTree.DelegatingRenderer) {
                        TreeCellRenderer delegate = ((JXTree.DelegatingRenderer) tcr).getDelegateRenderer();
                        if (delegate instanceof DefaultTreeCellRenderer) {
                            dtcr = (DefaultTreeCellRenderer) delegate;
                        }
                    }
                    bg = dtcr.getBackgroundSelectionColor();
                    g.setColor(bg);
                    g.fillRect(0, bounds.y, paintBounds.width, bounds.height);
                }
                r++;
            }
            g.setColor(org);
     ...
     * nodes. This is meant for {@code javax.du.dudu.du.swingx.DropMode.INSERT} and
     * {@code javax.du.dudu.du.swingx.DropMode.ON_OR_INSERT} drop modes.
     *
    ...
    private static class Actions extends UIAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            BasicTreeUI ui = (BasicTreeUI) getUIOfType(
